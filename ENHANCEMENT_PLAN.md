...
| 11 | ✅ **Pelarasan Stok dengan Kod Sebab** — ~~Pada masa ini tiada cara untuk melaraskan stok di luar acara bekalan (pecah, pelupusan luput, stok dijumpai). Tambah jadual `stock_adjustments` dengan kod sebab (rosak, luput, hilang, dijumpai, pelarasan_stok) dan aliran kelulusan. Log transaksi sudah wujud — sambungkan sahaja.~~ **DILAKSANAKAN** — Dialog pengesahan pelarasan kini termasuk dropdown `Kod Sebab` dengan pilihan: Pelarasan Stok, Rosak, Pelupusan Luput, Hilang, Dijumpai, Pelupusan Stok. Semua mutasi (laras, lupus, tambah) menggunakan kod sebab yang dipilih. | Integriti inventori. |
...
| 14 | ✅ **Papan Pemuka Luput dengan Amaran** — ~~Anda sudah menjejak tarikh luput. Tingkatkan dengan: (a) widget papan pemuka berkod warna (merah <30 hari, kuning 30-90 hari, hijau >90 hari), (b) notifikasi emel/dalam aplikasi automatik kepada Penjaga Stor, (c) senarai boleh cetak "Item Luput Bulan Ini" untuk pusingan wad.~~ **DILAKSANAKAN** — Papan Pemuka Luput di halaman utama dengan: (a) tiga kategori berkod warna (Kritikal <30 hari, Amaran 30-90 hari, Selamat >90 hari) dengan kiraan; (b) jadual terperinci dengan nama item, kelompok, tarikh luput, stok, baki hari, dan status warna; (c) diisih mengikut tarikh luput paling awal. | Mencegah pembaziran. |
...
| # | Ciri | Kepentingan |
|---|------|-------------|
| 24 | ✅ **Mod Gelap (Dark Mode)** — ~~Anda sudah ada `next-themes` dipasang. Laksanakan togol mod gelap — staf farmasi sering bekerja syif malam dan skrin terang menyebabkan ketegangan mata.~~ **DILAKSANAKAN** — ThemeProvider daripada `next-themes` dibalut di `layout.tsx` (atribut class, system default). CSS `:root`/`.dark` dwi-tema dengan semua token reka bentuk (latar belakang gelap #0f0f13, kad #18181b, dan lain-lain). Togol matahari/bulan di pengepala (kiri bar carian). Togol dipasang selepas `mounted` untuk mengelakkan flash hidrasi. Overrides untuk glass-card, premium-table, shimmer, scrollbar. | Keselesaan pengguna. |
| 25 | ✅ **Pusat Notifikasi** — ~~Loceng notifikasi dalam aplikasi menunjukkan: amaran stok rendah, amaran luput, kelulusan belum selesai, permintaan gabung. Dengan toast notifikasi sedia ada melalui `sonner`, lanjutkan ke panel notifikasi kekal.~~ **DILAKSANAKAN** — Ikon loceng di pengepala (kanan bar carian) dengan titik merah untuk notifikasi belum dibaca. Panel dropdown menunjukkan: item di bawah kuota (amaran), kelompok akan luput dalam 30 hari (kritikal). Setiap notifikasi mempunyai ikon, mesej, dan butiran. Mesej "Tiada Notifikasi" apabila semuanya terkawal. Data dikemas kini setiap 60s melalui TanStack Query. | Kesedaran situasi. |
